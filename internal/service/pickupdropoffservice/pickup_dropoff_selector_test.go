...
func TestPickupDropoffSelector_GetPickupDropoffPointsAndDurations(t *testing.T) {
	...
	pickup := model.NewPathPoint(*pickupCoord, enums.Pickup, now, nil)
	dropoff := model.NewPathPoint(*dropoffCoord, enums.Dropoff, later, nil)
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedDropoffWalking: 5 * time.Minute, // Same value since we're using a simple mock
			expectError:            false,
		...
		},
		{
			...
			expectedDropoff:        dropoff,
			expectedPickupWalking:  5 * time.Minute,
			expectedDropoffWalking: 7 * time.Minute,
			expectError:            false,
			expectCacheHit:         true,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.prePopulateCache {
				...
				cacheValue := pickupdropoffcache.NewValue(tc.expectedPickup, tc.expectedDropoff, tc.expectedPickupWalking, tc.expectedDropoffWalking)
				tc.cache.Set(cacheKey, cacheValue)
			}

			// Create walking time calculator with the mock routing engine
			walkingTimeCalculator := NewWalkingTimeCalculator(tc.routingEngine)

			// Create selector
			selector := NewPickupDropoffSelector(tc.generator, walkingTimeCalculator, tc.cache)
			...
			if result.PickupWalkingDuration() != tc.expectedPickupWalking {
				t.Errorf("Expected pickup walking duration %v but got %v", tc.expectedPickupWalking, result.PickupWalkingDuration())
			}

			// Check dropoff walking duration
			if result.DropoffWalkingDuration() != tc.expectedDropoffWalking {
				t.Errorf("Expected dropoff walking duration %v but got %v", tc.expectedDropoffWalking, result.DropoffWalkingDuration())
			}
		})
	}
}
...
func TestPickupDropoffSelector_GetPickupDropoffPointsAndDurations_CacheUsage(t *testing.T) {
	...
	// Create walking time calculator with the mock routing engine
	walkingTimeCalculator := NewWalkingTimeCalculator(routingEngine)

	// Create cache
	cache := pickupdropoffcache.NewPickupDropoffCache()

	// Create selector
	selector := NewPickupDropoffSelector(generator, walkingTimeCalculator, cache)
