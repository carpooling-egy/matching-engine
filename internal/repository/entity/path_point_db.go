...
type PathPointDB struct {
	...
	PointType           models.PointType `gorm:"column:type;type:point_type;not null"`
...
func (p *PathPointDB) ToPathPoint() *models.PathPoint {
    // Use the constructor for Coordinate
    coordinate, _ := models.NewCoordinate(p.Latitude, p.Longitude)
    
    var riderRequest *models.RiderRequest = p.RiderRequest.ToRiderRequest()
    
    // Fix: Call the constructor function properly
    return models.NewPathPoint(
        *coordinate,
        p.PointType,
